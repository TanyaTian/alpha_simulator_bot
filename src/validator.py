...
supported_functions = {
    ...
    'right_tail': {'min_args': 1, 'max_args': 2, 'arg_types': ['expression', 'expression'], 'param_names': ['x', 'minimum']},
    'bucket': {'min_args': 1, 'max_args': 5, 'arg_types': ['expression', 'expression', 'expression', 'expression', 'expression'], 'param_names': ['x', 'range', 'buckets', 'skipBoth', 'NaNGroup']},  # 第二个参数可以是string类型的range参数
    'tail': {'min_args': 1, 'max_args': 4, 'arg_types': ['expression', 'expression', 'expression', 'expression'], 'param_names': ['x', 'lower', 'upper', 'newval']},
    'left_tail': {'min_args': 1, 'max_args': 2, 'arg_types': ['expression', 'expression'], 'param_names': ['x', 'maximum']},
    ...
    'clamp': {'min_args': 1, 'max_args': 5, 'arg_types': ['expression', 'expression', 'expression', 'boolean', 'expression'], 'param_names': ['x', 'lower', 'upper', 'inverse', 'mask']},
    ...
    def t_IDENTIFIER(self, t):
        ...
        if t.value.lower() in {'true', 'false'}:
            ...
            # 如果后面跟着'('，则为函数名
                ...
                t.value = t.value.lower()  # 转换为小写以保持一致性
            # 检查是否为参数名（支持更多参数名）
            elif t.value in {'std', 'k', 'lambda_min', 'lambda_max', 'target_tvr', 'range', 'buckets', 'lag', 'rettype', 'mode', 'nth', 'constant', 'percentage', 'driver', 'sigma', 'rate', 'scale', 'filter', 'lower', 'upper', 'target', 'dest', 'event', 'sensitivity', 'force', 'h', 't', 'period', 'stddev', 'factor', 'k', 'useStd', 'limit', 'gaussian', 'uniform', 'cauchy', 'newval', 'skipBoth', 'NaNGroup', 'inverse', 'mask', 'maximum', 'minimum'}:
                t.type = 'IDENTIFIER'
            # 检查是否为函数名（不区分大小写）
